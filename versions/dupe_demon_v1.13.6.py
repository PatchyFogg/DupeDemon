...
def build_gui():
    ...
    class App(_TkBase):
        ...
        # ---------------- folder management ----------------
        def add_folder(self):
            # Tk's askdirectory has no native multi-select — it can only
            # return one folder per call, on every platform. The standard
            # workaround is to re-open the dialog immediately after each
            # pick so the user can add several folders back-to-back; Cancel
            # ends the run.
            added = 0
            initial = None
            while True:
                kwargs = {"title": "Choose a folder to scan (Cancel when done)"}
                if initial:
                    kwargs["initialdir"] = initial
                folder = filedialog.askdirectory(**kwargs)
                if not folder:
                    break
                initial = os.path.dirname(folder)
                if folder not in self._folder_paths_set():
                    self._add_folder_entry(folder, "source")
                    added += 1
            if added:
                self.status_label.config(
                    text=f"Added {added} folder{'s' if added != 1 else ''}.")
        ...
        def _render_next_group(self):
            ...
            if self._render_queue or self._pending_groups:
                self.status_label.config(
                    text=f"Processing {self._rendered_count} of "
                         f"{self._groups_total} groups…")
                if not self._render_queue and self._pending_groups:
                    self._render_more()
            else:
