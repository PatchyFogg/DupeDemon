    ...
    class PreferencesWindow(tk.Toplevel):
        ...
        def _clear_cache(self):
            ...
            for suffix in ("", "-wal", "-shm"):
                try:
                    Path(str(CACHE_FILE) + suffix).unlink()
                except OSError:
                    pass
            self._refresh_cache_button()
    ...
    class App(_TkBase):
        ...
        def _register_drop_targets(self):
            if not _DND_AVAILABLE:
                return
            self._folder_list_default_bg = self.folder_list.cget("background")
        ...
        def _on_drop_enter(self, event):
            try:
                self.folder_list.configure(background="#d6ecff")
            except Exception:
                pass
            return event.action
        ...
        def _on_drop_leave(self, event):
            try:
                self.folder_list.configure(background=self._folder_list_default_bg)
            except Exception:
                pass
            return event.action
            ...
            for raw in paths:
                ...
                if os.path.isdir(p):
                    ...
                    if parent and parent not in seen:
                        ...
                        self.folder_list.insert("end", parent)
                        seen.add(parent)
        ...
        def _open_compare_window(self, group, group_index):
            from datetime import datetime
            win = tk.Toplevel(self)
            win.title(f"Compare — Group {group_index} · {len(group)} files")
            keeper = pick_keeper(group, self.keep_var.get())

            # per-image width: fits within screen, capped to 460
            screen_w = win.winfo_screenwidth()
            per_w = max(220, min(460, (screen_w - 120) // max(len(group), 1)))

            outer = ttk.Frame(win, padding=14)
            outer.pack(fill="both", expand=True)

            canvas = tk.Canvas(outer, highlightthickness=0, height=per_w + 240)
            ...
            for fi in group:
                col = ttk.Frame(strip, padding=(0, 0, 12, 0))
                col.pack(side="left", anchor="n")

                is_keeper = fi.path == keeper.path
                border_color = "#2ea043" if is_keeper else "#c8c8c8"
                wrap = tk.Frame(col, background=border_color, padx=3, pady=3)
                wrap.pack()

                photo = self._make_compare_image(fi.path, per_w)
                if photo is not None:
                    img_lbl = tk.Label(wrap, image=photo, background=border_color,
                                       cursor="hand2")
                    img_lbl.pack()
                    ...
                    tk.Label(wrap, text="(preview failed)",
                             width=per_w // 8, height=per_w // 20,
                             background=border_color).pack()
            ...
            win.update_idletasks()
            req_w = min(win.winfo_reqwidth() + 24, screen_w - 60)
            req_h = min(win.winfo_reqheight() + 24, win.winfo_screenheight() - 100)
            x = (screen_w - req_w) // 2
            y = max(30, (win.winfo_screenheight() - req_h) // 2 - 20)
            win.geometry(f"{req_w}x{req_h}+{x}+{y}")
            win.lift()
            win.focus_set()

        def _make_compare_image(self, path, max_dim):
            try:
                with Image.open(path) as img:
