...
def is_automation_permission_error(error_text):
    """True if a Finder AppleScript failure is macOS blocking automation
    (error -1743), not some other Finder/filesystem problem."""
    if not error_text:
        return False
    t = error_text.lower()
    return "-1743" in t or "not authorized to send apple events" in t
...
def build_gui():
    ...
    class PieProgress(tk.Canvas):
        def __init__(self, master, size=28, bg_color="#e0e0e0", fg_color="#34c759",
                     **kw):
        ...
        def _draw(self):
            ...
            if self._maximum > 0 and self._value > 0:
                ...
                if frac >= 1.0:
                    # Tk's create_arc renders nothing for an exact 360-degree
                    # extent (the pieslice math degenerates) — a full oval is
                    # the only reliable way to show 100% filled.
                    self.create_oval(pad, pad, pad + s, pad + s,
                                     fill=self._fg_color, outline="")
                else:
                    extent = frac * 360
    ...
    class App(_TkBase):
        ...
        def _prompt_automation_permission(self):
            win = tk.Toplevel(self)
            win.title("Finder Permission Needed")
            win.resizable(False, False)
            win.transient(self)
            body = ttk.Frame(win, padding=20)
            body.pack(fill="both", expand=True)
            ttk.Label(body, text="Dupe Demon needs permission to use Finder",
                      font=("", 14, "bold")).pack(anchor="w")
            ttk.Label(body, justify="left", wraplength=430, text=(
                "Moving files to the Trash works by asking Finder to do it "
                "(so it stays safe and recoverable). macOS is blocking that "
                "request — Dupe Demon doesn't have permission to control "
                "Finder.\n\n"
                "To enable: System Settings → Privacy & Security → "
                "Automation → Dupe Demon → check Finder. If Dupe Demon "
                "isn't listed yet, try Move to Trash again after opening "
                "Settings — macOS adds it to the list on the next attempt."
            )).pack(anchor="w", pady=(8, 14))
            buttons = ttk.Frame(body)
            buttons.pack(fill="x")
            ...
            ttk.Button(buttons, text="Later", command=win.destroy).pack(
                side="right", padx=(0, 8))
            open_btn = ttk.Button(buttons, text="Open System Settings…",
                                  command=open_now)
            open_btn.pack(side="right")
            open_btn.focus_set()
            win.bind("<Return>", lambda e: open_now())
            win.bind("<Escape>", lambda e: win.destroy())
            self._center_over_main(win)
        ...
        def add_folder(self):
            ...
            # this app already accepts multi-folder drops, so open a
            # medium-sized Finder window at a sensible starting point and
            # let the user drag several folders onto the list at once.
            # (Was 900x700 — big enough to look like something had crashed
            ...
            script = (
                ...
                "  activate\n"
                f'  set targetWindow to make new Finder window to (POSIX file "{escaped}" as alias)\n'
                "  set bounds of targetWindow to {150, 150, 750, 600}\n"
                "end tell"
            )
        ...
        def trash_selected(self):
            ...
            if error:
                if is_automation_permission_error(error):
                    self._prompt_automation_permission()
                else:
                    messagebox.showerror(
                        APP_NAME, f"Could not move files to Trash:\n{error}")
