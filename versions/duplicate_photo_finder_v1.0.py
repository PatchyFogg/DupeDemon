#!/usr/bin/env python3
"""
Duplicate Photo Finder for macOS
================================
...
Run:  python3 duplicate_photo_finder.py
Test: python3 duplicate_photo_finder.py --selftest
"""
...
APP_SUPPORT_DIR = Path.home() / "Library" / "Application Support" / "DuplicatePhotoFinder"
...
def build_gui():
    ...
    class App(tk.Tk):
        ...
        def _build_widgets(self):
            ...
            self.status_label = ttk.Label(progress_frame, text="Ready.", width=44, anchor="e")
            ...
            self.keep_var = tk.StringVar(value="Newest file")
            keep_menu = ttk.Combobox(
                actions, textvariable=self.keep_var, state="readonly", width=18,
                values=["Newest file", "Oldest file", "Largest file",
                        "Smallest file", "Highest resolution", "First in group"])
        ...
        def _scan_finished(self, groups, n_files):
            ...
            self._scan_reset(
                f"{n_files} images scanned — {len(groups)} groups, "
        ...
        def auto_select(self):
            if self._render_queue:
                messagebox.showinfo(APP_NAME, "Still rendering results — try again in a moment.")
                return
            rule = self.keep_var.get()
            keyfuncs = {
                ...
                "First in group": lambda f: 0,
            }
            key = keyfuncs[rule]
            for group in self.groups:
                if rule == "First in group":
                    keeper = group[0]
                else:
                    keeper = max(group, key=key)
                for fi in group:
                    var = self.check_vars.get(fi.path)
                    if var is not None:
                        var.set(fi.path != keeper.path)
        ...
        def trash_selected(self):
            ...
            if not messagebox.askyesno(
                    APP_NAME,
                    f"Move {len(selected)} files ({human_size(total)}) to the Trash?\n\n"
            ...
            self.status_label.config(text=f"Moved {len(selected)} files to the Trash.")
            self._remove_paths_from_results(trashed)

        def _remove_paths_from_results(self, removed):
            new_groups = []
            for group in self.groups:
            ...
            self.file_by_path.clear()
            if new_groups:
                self._render_queue = list(enumerate(new_groups, start=1))
            else:
