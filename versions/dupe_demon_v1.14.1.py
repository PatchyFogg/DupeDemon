...
def build_gui():
    ...
    class PieProgress(tk.Canvas):
        ...
        def stop_spinning(self):
            """Freeze the pie at its current value/color without resetting
            it to empty. The spin loop otherwise only stops when value is
            explicitly set to 0 — any caller that leaves a value in place
            (e.g. Stop mid-render, showing "first N of M") but never calls
            that leaves the pie spinning forever, since nothing else ever
            turns _spinning off."""
            self._spinning = False
            self._rotation = 0
            self._draw()
