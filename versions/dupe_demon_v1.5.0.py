    ...
    class App(tk.Tk):
        ...
        def stop_scan(self):
            ...
            if self._render_queue:
                rendered = self._render_total - len(self._render_queue)
                self._render_queue = []
                self.groups = self.groups[:rendered]
                self.stop_btn.state(["disabled"])
                self.status_label.config(
                    text=f"Stopped — showing first {rendered} of "
                         f"{self._render_total} groups.")
        ...
        def _start_rendering(self, groups, summary_text, track_progress=True):
            """Queue groups for display. track_progress=False keeps the bar as-is
            (e.g. after deletes, overall scan progress stays at 100%)."""
